...
$dm = new DataManager();
			...
			$s_id = escaped_var_from_post("s_id");// mysqli_real_escape_string($dm->connection, $_REQUEST["s_id"]);
			$s_supplier_id = escaped_var_from_post("s_supplier_id");
			$s_item_number = escaped_var_from_post("s_item_number");;
			$s_price = escaped_var_from_post("s_price");
			$s_price_per = escaped_var_from_post("s_price_per");
			$s_cost = escaped_var_from_post("s_cost");
			$s_inventory = escaped_var_from_post("s_inventory");
			$s_type = escaped_var_from_post("s_type");
			$s_style = escaped_var_from_post("s_style");
			$s_description = escaped_var_from_post("s_description");
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Id</td><td>Supplier</td><td>Item number</td><td>Price</td><td>Price_per</td><td>Cost</td><td>Inventory</td><td>Type</td><td>Style</td><td>Description</td>
			  ...
			  <td><input type="text" name="s_id"  value="<?php  echo $s_id  ?>"/></td>
				<td><?php  
					...
					$dd->set_name("s_supplier_id");
					$dd->set_selected_value($s_supplier_id);
					$dd->set_active_only(false);
					$dd->set_order("ASC");	
					$dd->display();
				?></td>
				<td><input type="text" name="s_item_number"  value="<?php  echo $s_item_number  ?>"/></td>
				<td><input type="text" name="s_price"  value="<?php  echo $s_price  ?>"/></td>
				<td><input type="text" name="s_price_per"  value="<?php  echo $s_price_per  ?>"/></td>
				<td><input type="text" name="s_cost"  value="<?php  echo $s_cost  ?>"/></td>
				<td><input type="text" name="s_inventory"  value="<?php  echo $s_inventory  ?>"/></td>
				<td><?php
					$dd->clear();  
				...
				<td><input type="text" name="s_description"  value="<?php  echo $s_description  ?>"/></td>
					  

				<input type="hidden" id="sort" name="sort" value="<?php echo $s_sort?>" />
				<input type="hidden" id="sort_dir" name="sort_dir" value="<?php echo $s_sort_dir?>" />
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where .= ' AND component_id = "'.$s_id.'"';
						} 
						if($s_supplier_id != ""){
								$query_where .= ' AND component_supplier_id = "'.$s_supplier_id.'"';
						} 
						if($s_item_number != ""){
								$query_where .= ' AND component_item_number = "'.$s_item_number.'"';
						} 
						if($s_price != ""){
								$query_where .= ' AND component_price = "'.$s_price.'"';
						} 
						if($s_price_per != ""){
								$query_where .= ' AND component_price_per = "'.$s_price_per.'"';
						} 
						if($s_cost != ""){
								$query_where .= ' AND component_cost = "'.$s_cost.'"';
						} 
						if($s_inventory != ""){
								$query_where .= ' AND component_inventory = "'.$s_inventory.'"';
						} 
						if($s_type != ""){
								$query_where .= ' AND component_type = "'.$s_type.'"';
						} 
						if($s_style != ""){
								$query_where .= ' AND component_style = "'.$s_style.'"';
						} 
						if($s_description != ""){
								$query_where .= ' AND component_description = "'.$s_description.'"';
						}		

						$query = "SELECT * from component 
						LEFT JOIN componenttype ON component.component_type = componenttype.componentType_id
						LEFT JOIN supplier ON component.component_supplier_id = supplier.supplier_id
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/component_list_template.htm');
