 ...
 class Orderactivity {
	...
	function __construct() {
				...
				public function get_id() { return $this->id;}
		 		...
		 		public function set_orderId($value) {$this->orderId=$value;}
...
public function save() {
		...
		try{
			...
			if($this->get_id() == 0) {
				...
				$strSQL = "INSERT INTO orderactivity (orderActivity_id, orderActivity_content, orderActivity_orderId, is_active, orderActivity_date_created, orderActivity_last_updated, orderActivity_last_updated_user) 
        VALUES (
								'".mysqli_real_escape_string($dm->connection, $this->get_id())."',
								'".mysqli_real_escape_string($dm->connection, $this->get_content())."',
								'".mysqli_real_escape_string($dm->connection, $this->get_orderId())."',
				...
				$strSQL = "UPDATE orderactivity SET 
								orderActivity_content='".mysqli_real_escape_string($dm->connection, $this->get_content())."',						 
						 		orderActivity_orderId='".mysqli_real_escape_string($dm->connection, $this->get_orderId())."',						 
						 		is_active='".mysqli_real_escape_string($dm->connection, $this->get_active())."',						 
						 		orderActivity_last_updated=NOW(),						
						 		orderActivity_last_updated_user='".mysqli_real_escape_string($dm->connection, $this->get_last_updated_user())."'
							
						 	WHERE orderActivity_id=".mysqli_real_escape_string($dm->connection, $this->get_id());
			...
			include_once(CLASS_FOLDER . 'class_error_handler.php');
	...
	public function delete_by_id($id) {
		...
		//	require_once($class_folder . '/class_data_manager.php');
			...
			$strSQL = "DELETE FROM orderactivity WHERE orderActivity_id=" . $id;
			$result = $dm->updateRecords($strSQL);
			return $result;
		}
		catch(Exception $e) {
	...
	public function get_by_id($id) {
		...
		//	require_once($class_folder . '/class_data_manager.php');
			$status = false;
			$dm = new DataManager();
			$strSQL = "SELECT * FROM orderactivity WHERE orderActivity_id=" . $id;
  ...
  private function load($row){
	$this->set_id($row["orderActivity_id"]);
				$this->set_content($row["orderActivity_content"]);
				$this->set_orderId($row["orderActivity_orderId"]);
				$this->set_active($row["is_active"]);
				$this->set_date_created($row["orderActivity_date_created"]);
				$this->set_last_updated($row["orderActivity_last_updated"]);
				$this->set_last_updated_user($row["orderActivity_last_updated_user"]);
				  }
}