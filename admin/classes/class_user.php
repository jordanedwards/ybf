...
//	public function set_tt_count($value) { $this->first_name = $value; }
	...
	public function set_password($value, $hashed = false) { require($_SERVER['DOCUMENT_ROOT'] . '/admin/includes/config_secure.php'); $this->password = ($hashed ? $value : hash("sha256", trim($value) . $appConfig["salt"])); }
	...
	public function save() {
		...
		try{
			...
			if($this->get_id() == "0") {
				...
				$strSQL = "INSERT INTO user (user_first_name, user_last_name, user_tel, user_email, user_password, user_login, user_role, user_date_created, user_last_updated, user_last_updated_user) VALUES ('".mysqli_real_escape_string($dm->connection, $this->get_first_name())."' ,'".mysqli_real_escape_string( $dm->connection, $this->get_last_name())."' ,'".mysqli_real_escape_string($dm->connection,  $this->get_tel())."','".mysqli_real_escape_string($dm->connection, $this->get_email())."' ,'". mysqli_real_escape_string($dm->connection, $this->get_password()) ."', NOW(),".mysqli_real_escape_string($dm->connection, $this->get_role()).",NOW(),NOW(),'".mysqli_real_escape_string($dm->connection, $this->get_last_updated_user())."')";

			}
			else {
				$strSQL = "UPDATE user SET user_first_name= '".mysqli_real_escape_string($dm->connection, $this->get_first_name())."',user_last_name= '".mysqli_real_escape_string($dm->connection, $this->get_last_name())."',user_tel= '".mysqli_real_escape_string($dm->connection, $this->get_tel())."', user_email= '".mysqli_real_escape_string($dm->connection, $this->get_email())."', user_password = '".mysqli_real_escape_string($dm->connection, $this->get_password())."', user_role = ".mysqli_real_escape_string($dm->connection, $this->get_role()).", user_last_updated=NOW(), user_last_updated_user='".mysqli_real_escape_string($dm->connection, $this->get_last_updated_user())."' WHERE user_id=".mysqli_real_escape_string($dm->connection, $this->get_id());
			}
