...
<div class="" style="margin-left: initial;">
      ...
      <div class="row">
        ...
        <i class="fa fa-plus-circle"></i> <a href="customers_edit.php?id=0">Add New Item</a></p>
<?php 
...
$dm = new DataManager();
			...
			$s_use_pricegrid = escaped_var_from_post("s_use_pricegrid");
			$s_category =escaped_var_from_post("s_category");
 ...
 <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Specialitems Id</td><td>Specialitems Name</td><td>Specialitems Price</td><td>Specialitems Use_pricegrid</td><td>Specialitems Category</td><td>Specialitems Active</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
			  ...
			  <td><input type="text" name="s_id"  value="<?php  echo $s_id  ?>"/></td>
				...
				<td><input type="text" name="s_use_pricegrid"  value="<?php  echo $s_use_pricegrid  ?>"/></td>
				<td><?php  
					$dd->set_table("specialitemcategories");
					$dd->set_name_field("specialItemCategories_name");
					$dd->set_name("s_category");
					$dd->set_selected_value($s_category);
...
<?php 
	...
	// Page set to reload (new query)		
		...
		if($s_id != ""){
				$query_where .= ' AND specialitems_id = "'.$s_id.'"';
		} 
		if($s_name != ""){
				$query_where .= ' AND specialitems_name = "'.$s_name.'"';
		} 
		if($s_price != ""){
				$query_where .= ' AND specialitems_price = "'.$s_price.'"';
		} 
		if($s_use_pricegrid != ""){
				$query_where .= ' AND specialitems_use_pricegrid = "'.$s_use_pricegrid.'"';
		} 
		if($s_category != ""){
				$query_where .= ' AND specialitems_category = "'.$s_category.'"';
		} 
		if($s_active != ""){
				$query_where .= ' AND specialitems_active = "'.$s_active.'"';
		}		
	...
	$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/specialitems_list_template.htm');
