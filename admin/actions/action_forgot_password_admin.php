<?php
    ...
    include($_SERVER['DOCUMENT_ROOT'] . "/admin/classes/class_user.php");
	$user = new User();

  //Initiate the emailer
	include($_SERVER['DOCUMENT_ROOT'] . '/admin/includes/config_mail.php');
	require_once($_SERVER['DOCUMENT_ROOT']."/admin/classes/class_phpmailer.php");
	$mail = new PHPMailer();
	$mail->IsSMTP();
...
<a href='http://".$_SERVER['HTTP_HOST']."/admin/actions/action_forgot_password_admin.php?id=".$code."'>http://".$_SERVER['HTTP_HOST']."/admin/actions/action_forgot_password_admin.php?id=".$code."</a><br />
<br />
Please do not reply to this email.<br />
".$appConfig["app_title"]." Team<br />";
			$mail->Body = $body;
			
			mail($_POST['email'], "Password Reset", $body);
			if(!$mail->Send()){
		...
		if($user->get_by_id(intval($_GET['id'])) && $_GET['c'] == $user->get_password()) {
			...
			mail("jordan@orchardcity.ca","test",$password);
			$user->save();
...
".$appConfig["app_title"]." Team<br />";
			$mail->Body = $body;
			mail($user->get_email(), "Password reset",$body);
			if(!$mail->Send()){
				die("Message could not be sent. <p>Mailer Error: " . $mail->ErrorInfo);
			}
			$mail = NULL;
